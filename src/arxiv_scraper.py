...
class ArxivScraper:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.cache_file = self.storage_config.get(
            "cache_file", os.path.join(self.data_dir, "cache.json")
        )
        try:
        ...
        try:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    self.logger.warning("ArXiv 返回空页面，可能已获取全部结果")
                    break
                    ...
                    if retry_count < max_retries:
                        ...
                        self.logger.warning(
                            f"ArXiv API 错误: {e}，{wait_time}秒后重试 ({retry_count}/{max_retries})"
                        )
    ...
    def _calculate_relevance_scores(
        ...
        for paper in papers:
            try:
                ...
                self.logger.warning(
                    f"计算相关度评分失败 ({paper.get('arxiv_id', 'unknown')}): {str(e)}"
                )
    ...
    def _filter_processed_papers(
        ...
        for paper in papers:
            ...
            has_summary = (
                enable_summary
                and cache_entry.get("ai_summary", {}).get("status") == "success"
            )
            has_translation = (
                enable_translation
                and bool(cache_entry.get("translation"))
                and not str(cache_entry.get("translation", "")).startswith("翻译失败")
            )
            has_insights = (
                enable_insights
                and cache_entry.get("insights", {}).get("status") == "success"
            )

            # 如果缓存有效且所有需要的内容都已缓存，则跳过
            if cache_valid and (
                (enable_summary and has_summary or not enable_summary)
                and (enable_translation and has_translation or not enable_translation)
                and (enable_insights and has_insights or not enable_insights)
            ):
                skipped_count += 1
                self.logger.debug(
                    f"跳过已处理论文: {paper.get('title', 'N/A')[:50]}..."
                )
            else:
                unprocessed_papers.append(paper)
        ...
        if skipped_count > 0:
            self.logger.info(
                f"已筛除 {skipped_count} 篇已处理论文：{len(papers)} → {len(unprocessed_papers)} 篇"
            )
    ...
    def _get_prompts_signature(self) -> str:
        ...
        prompts_path = self.config.get("ai", {}).get(
    ...
    def _save_cache(self) -> None:
        ...
        try:
            ...
            payload = {"version": 1, "items": self.cache}
    ...
    def _apply_multi_level_sort(
        ...
        for sort_config in reversed(multi_sort):
            ...
            try:
                papers = sorted(
                    papers, key=lambda p: get_sort_key(p, field), reverse=reverse
                )
    ...
    def _save_as_json(
        self, papers: List[Dict[str, Any]], timestamp: str
    ) -> Optional[str]:
        """保存为 JSON 格式，返回文件路径"""
    ...
    def filter_papers_with_ai(
        ...
        self.logger.info(
            f"开始 AI 智能筛选论文（关键词：{filter_keywords}，阈值：{filter_threshold}）..."
        )
        ...
        for paper in papers:
            ...
            if (
                cache_entry
                and cache_entry.get("filter_cache_key") == self.filter_cache_key
            ):
        ...
        if papers_to_filter:
            ...
            if max_workers == 1:
                for i, paper in enumerate(papers_to_filter, 1):
                    try:
                        self.logger.info(
                            f"[{i}/{len(papers_to_filter)}] 筛选: {paper['title'][:50]}..."
                        )
                ...
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    future_to_paper = {
                        executor.submit(
                            ai_service.filter_paper, paper, filter_keywords
                        ): paper
        ...
        for paper in papers:
            ...
            if (
                filter_result.get("relevant", False)
                and filter_result.get("confidence", 0.0) >= filter_threshold
            ):
                filtered_papers.append(paper)
    ...
    def process_with_ai(self, papers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for paper in papers:
            cache_entry = self._get_cache_entry(paper)
            cache_valid = (
                cache_entry and cache_entry.get("ai_cache_key") == self.ai_cache_key
            )
        ...
        def process_single(task):
            ...
            if do_translation:
                result["translation"] = ai_service.translate_text(
                    paper.get("summary", "")
                )
            if do_insights:
        ...
        if tasks:
            ...
            if max_workers == 1:
                for i, task in enumerate(tasks, 1):
                    ...
                    try:
                        self.logger.info(
                            f"[{i}/{len(tasks)}] 处理: {paper['title'][:50]}..."
                        )
                    ...
                    if "translation" in result and not str(
                        result["translation"]
                    ).startswith("翻译失败"):
                        self._update_cache_entry(
                            paper, translation=result["translation"]
                        )
                ...
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    future_to_task = {
                        executor.submit(process_single, task): task for task in tasks
                    }
                    ...
                    for future in as_completed(future_to_task):
                        ...
                        if result.get("ai_summary", {}).get("status") == "success":
                            self._update_cache_entry(
                                paper, ai_summary=result["ai_summary"]
                            )
                        if "translation" in result and not str(
                            result["translation"]
                        ).startswith("翻译失败"):
...
"""
                        ...
                        attachments = [{"file_path": filepath, "filename": filename}]
                        ...
                        success = send_report_via_webhook(
                            webhook_config, markdown_content, len(papers), timestamp
                        )
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if papers:
                ...
                email_sent = markdown_path is None and self.config.get("ai", {}).get(
